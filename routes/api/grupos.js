...
router.get('/search', (req, res) => {
    ...
    const query = Grupo
        .aggregate([
            {$match: {
                $or: [
                {'nombre': {$regex: searchInput, $options: 'i'}},
                {'codigo': {$regex: searchInput, $options: 'i'}},
                {'vida': {$regex: searchInput, $options: 'i'}},
            ]}},
        ])

        Grupo.aggregatePaginate(query, {
            limit: 5,
